...
void CFHE_Test::StartTimer() { t0 = timeNow(); }
...
void CFHE_Test::SetNumTest(uint n) { num_test = n; }

uint CFHE_Test::GetVerbosity() { return verbosity; }

void CFHE_Test::SetVerbosity(uint v) { verbosity = (v <= 4) ? v : 4; }

bool CFHE_Test::GetTestFresh() { return test_fresh; }

void CFHE_Test::SetTestFresh(bool val) { test_fresh = val; }

bool CFHE_Test::GetRegenerateKeys() { return regenerate_keys; }

void CFHE_Test::SetRegenerateKeys(bool val) { regenerate_keys = val; }

uint CFHE_Test::CreateRandomNumber() { return dis(gen); }
...
void CFHE_Test::PrintTestReport(TestReport report, int64_t n, int64_t result,
    ...
    if (verbosity >= 2 && report.test_result == TR_FAIL ||
        verbosity >= 3 && report.test_result == TR_SUCCESS) {
        cout << "n: " << n << " result: " << result << " expected: " << expected
             << endl;
    }
}

void CFHE_Test::PrintTestReport(TestReport report, int64_t n1, int64_t n2,
    ...
    if (verbosity >= 2 && report.test_result == TR_FAIL ||
        verbosity >= 3 && report.test_result == TR_SUCCESS) {
        cout << "n1: " << n1 << " n2: " << n2 << " result: " << result
...
void CFHE_Test::StartNoiseTest() {
    ...
    for (uint trial = 1; trial <= num_test; trial++) {
        ...
        //
        LWECiphertext ct = cfhe_base->GetBinFHEContext().Encrypt(
            cfhe_base->GetLWEPrivateKey(), CreateRandomNumber() % 2, FRESH);
        //
        StartTimer();
        ...
        //
        if (verbosity >= 1) {
            cout << "Trial #" << trial << ": fresh: " << noise
                 << " bs: " << noise_bs << " delta_t: " << delta_t << " ms"
                 << " avg_time: " << avg_t << " ms" << endl;
        }
    }
